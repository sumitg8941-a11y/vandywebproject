...
// Track offer engagement stats (time + pages)
app.post('/api/track/offer-stats/:id', async (req, res) => {
    const { id } = req.params;
    if (!validateId(id)) return res.status(400).json({ error: 'Invalid ID format' });
    try {
        ...
        await Offer.findOneAndUpdate(
            { id: id.toLowerCase() },
            { $inc: { totalTimeSeconds: duration || 0 }, $max: { maxPagesViewed: maxPage || 0 } }
        );
        res.status(200).send('OK');
    } catch (e) { res.status(500).send('Error'); }
});
...
app.post('/api/offer/:id/rate', async (req, res) => {
    ...
    try {
        ...
        const offer = await Offer.findOne({ id: id.toLowerCase() });
        if (!offer) return res.status(404).json({ error: 'Offer not found' });
        const newCount = offer.ratingCount + 1;
        const newRating = ((offer.rating * offer.ratingCount) + rating) / newCount;
        offer.rating = newRating;
        offer.ratingCount = newCount;
        await offer.save();
        await SiteStat.findOneAndUpdate(
            { id: 'global' },
            { $inc: { totalRatings: 1 }, $set: { avgRating: newRating } },
            { upsert: true }
        );
